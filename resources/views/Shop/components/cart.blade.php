@if(session('cart'))
    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="row" style="margin-bottom: 20px">
            <div class="col-lg-6">
                <a href="/"><button style="color: black; background:white; border-color: black;" class="btn btn-default" type="submit">Tiếp Tục Mua Hàng</button></a>
            </div><!-- /.col-lg-6 -->
            <div class="col-lg-6">
                <form action="{{ route('shop.cart.check-coupon') }}" method="get">
                    <div class="input-group">
                        {{-- <select name="coupon_code" id="coupon_code" style="width: 200px; float: right; height: 34px;">
                            @foreach ($datas as $data)
                                <option value="{{$data->code}}">{{$data->code}}</option>
                            @endforeach
                        </select> --}}
                        {{-- <select name="coupon_code2" id="coupon_code2" style="width: 200px; float: right; height: 34px;">
                            <option value="">--Chọn mã giảm giá 2--</option>
                            @foreach ($datas as $data)
                                <option value="{{$data->code}}">{{$data->code}}</option>
                            @endforeach
                        </select> --}}
                ...
                @endif --}}
                @error('errorCoupon')
                    <p style="text-align: right;color: red;">{{ $message }}</p>
                @enderror
            </div><!-- /.col-lg-6 -->
        </div><!-- /.row -->
    </div>
                        ...
                        <td class="cart-unit">
                            <ul class="price text-right">
                                @if($product['item']->sale==0)
                                    <span class="price">{{number_format($product['item']->price)}} <sup>đ</sup> </span>
                                @else
                                    <span class="price" style="color: red; ">{{number_format($product['item']->sale)}} <sup>đ</sup></span>
                                    &nbsp;
{{--                                    <span class="old-price">{{number_format($product['item']->price)}}<sup>đ</sup></span>--}}
                                @endif
                            </ul>
                        </td>
                        ...
                        <td class="cart-total">
                            ...
                            @else
                            <span class="price">{{ number_format($product['qty'] * $product['item']->sale ,0,",",".") }}đ</span>
